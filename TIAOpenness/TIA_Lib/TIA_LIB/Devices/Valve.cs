...
namespace TIA_LIB.Devices
{
    public class Valve : GeneratedObject
    {
        public Valve(XmlUnit unit, string tagName, int iconType = 0, int interlockCount = 0, int SafeInterlockCount = 0, bool mon_opn = false, bool mon_cls = false, bool mon_const = false, bool qualityBit = true, bool neg = true, int tp_number = -1, int mon_t = -1) : base(unit, tagName)
        {
            ...
            if (network != null) hasLock = network.FindCall("fbIntlck7", "FB", "LocalVariable", tagName + "_Interlock1");
            ...
            if (network == null || network != null && (interlockCount != 0 || SafeInterlockCount != 0) && hasLock == null)
            {
                if(network != null && (interlockCount != 0 || SafeInterlockCount != 0) && hasLock == null)
                {
                    ...
                    var deleteWire = networkSource.Descendants(_Namespace + "NameCon").Where(el => el.Attribute("Name").Value != null && el.Attribute("Name").Value == "en").FirstOrDefault();
                    if (deleteWire != null) deleteWire.Parent.Remove();

                    if (interlockCount != 0)
                    {
                        deleteWire = networkSource.Descendants(_Namespace + "NameCon").Where(el => el.Attribute("Name").Value != null && el.Attribute("Name").Value == "LOCK").FirstOrDefault();
                        if (deleteWire != null) deleteWire.Parent.Remove();
                    }

                    if (SafeInterlockCount != 0)
                    {
                        deleteWire = networkSource.Descendants(_Namespace + "NameCon").Where(el => el.Attribute("Name").Value != null && el.Attribute("Name").Value == "S_LOCK").FirstOrDefault();
                        if (deleteWire != null) deleteWire.Parent.Remove();
                    }
                }
                ...
                if (interlockCount != 0 || SafeInterlockCount != 0)
                {
                    ...
                    interlock = network.FindCall("fbIntlck7", "FB", "LocalVariable", tagName + "_Interlock" + (interlockCount + SafeInterlockCount));
            ...
            network.SetParameter(call, "CFG", "Input", "typeValveCfg", "cfgData|" + unit.Name + "_" + tagName, true);
            network.SetParameter(call, "RESET", "Input", "Bool", "HMI|" + "RESET", true);
            network.SetParameter(call, "PROT", "Input", "Bool", "F_Safe|" + "HSS", true);

            network.SetParameter(call, "MON_OPN", "Input", "Bool", mon_opn.ToString());
            network.SetParameter(call, "MON_CLS", "Input", "Bool", mon_cls.ToString());
            ...
            if (mon_opn || mon_cls)
            {
                if (mon_cls || mon_opn)
                {
                    ...
                    var checkWire = networkSource.Descendants(_Namespace + "Wire").Where(el => el.Elements().Count() == 2 &&
                                                                                                            el.Elements().ElementAtOrDefault(0).Attribute("Name") != null &&
                                                                                                            el.Elements().ElementAtOrDefault(1).Attribute("Name") != null &&
                                                                                                            el.Elements().ElementAtOrDefault(0).Attribute("Name").Value == "out" &&
                                                                                                            el.Elements().ElementAtOrDefault(1).Attribute("Name").Value == "FB_OPN").FirstOrDefault();

                    if (checkWire == null)
                    {
                        var deleteWire = networkSource.Descendants(_Namespace + "NameCon").Where(el => el.Attribute("Name").Value != null && el.Attribute("Name").Value == "FB_OPN").FirstOrDefault();
                        if (deleteWire != null) deleteWire.Parent.Remove();
                    ...
                    checkWire = networkSource.Descendants(_Namespace + "Wire").Where(el => el.Elements().Count() == 2 &&
                                                                                                        el.Elements().ElementAtOrDefault(0).Attribute("Name") != null &&
                                                                                                        el.Elements().ElementAtOrDefault(1).Attribute("Name") != null &&
                                                                                                        el.Elements().ElementAtOrDefault(0).Attribute("Name").Value == "out" &&
                                                                                                        el.Elements().ElementAtOrDefault(1).Attribute("Name").Value == "FB_CLS").FirstOrDefault();

                    if (checkWire == null)
                    {
                        var deleteWire = networkSource.Descendants(_Namespace + "NameCon").Where(el => el.Attribute("Name").Value != null && el.Attribute("Name").Value == "FB_CLS").FirstOrDefault();
                        if (deleteWire != null) deleteWire.Parent.Remove();
                ...
                if (mon_cls && mon_opn && qualityBit)
                {
                    ...
                    var checkWire = networkSource.Descendants(_Namespace + "Wire").Where(el => el.Elements().Count() == 2 &&
                                                                                                        ...
                                                                                                        el.Elements().ElementAtOrDefault(1).Attribute("Name") != null &&
                                                                                                        el.Elements().ElementAtOrDefault(0).Attribute("Name").Value == "out" &&
                                                                                                        el.Elements().ElementAtOrDefault(1).Attribute("Name").Value == "ERR_EXT").FirstOrDefault();
                    if (checkWire == null)
                    {
                        var deleteWire = networkSource.Descendants(_Namespace + "NameCon").Where(el => el.Attribute("Name").Value != null && el.Attribute("Name").Value == "ERR_EXT").FirstOrDefault();
                        if (deleteWire != null) deleteWire.Parent.Remove();
                ...
                else if ((mon_cls || mon_opn) && qualityBit)
                {
            ...
            else
            {
                if (tp_number != -1)
                {
                    network.SetParameter(call, "MON_T_ST", "Input", "Real", "TP" + tp_number.ToString() + "|OUT", true, true);
                    network.SetParameter(call, "MON_T_DN", "Input", "Real", "TP" + tp_number.ToString() + "|OUT", true, true);
